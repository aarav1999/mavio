...
└── frontend/                       — the Next.js app (deploy this on Vercel)
    ...
    ├── lib/
    │   ├── ai/                     — gemini.ts (Groq wrapper) + embeddings.ts
...
There is no `deployment/` folder, no FastAPI backend, no microservices. Everything ships from `frontend/`.
...
| **IMAP (Yahoo, AOL, iCloud, custom)** | Encrypted password (AES-256-GCM) | yes (envelope-fast + body on demand) | yes (SMTP via nodemailer) | read/unread via `\Seen`; archive/star/trash handled DB-side | DB-side full-text | full enough for MVP — see `lib/providers/imap.ts:222-244` for the documented limitations |

The plugin classes in `frontend/plugins/` are thin wrappers over the implementations in `frontend/lib/providers/` and exist so the Agent OS plugin registry can list/describe them uniformly.
...
**Hooks (3)** in `frontend/hooks/`:
- `onEmailReceived`, `onAnalysisComplete`, `onReplyGenerated`
- Today these are **instrumentation hooks** (console logging + optional callback). Documented as the integration seam for future side-effects (notifications, webhooks).

**Plugins (3)** in `frontend/plugins/`:
- `GmailPlugin`, `OutlookPlugin`, `ImapPlugin` — all `implements EmailProvider`

**Orchestrator** in `frontend/lib/orchestrator.ts`:
- `processEmail()` runs all five core agents sequentially.
...
| Decision | Rationale |
|---|---|
| **Monolithic Next.js** instead of microservices | Single dev, fast iteration, no service mesh. Everything lives in `frontend/`. |
...
- **Hugging Face inference** — free tier; embeddings cached per email row.
...
- **Jest unit tests** — agents (mocked Groq), skills, encryption (incl. tag/IV tamper tests), orchestrator (full mock), provider factory, IMAP, utils. **75 tests across 14 suites, all green.**
- **Playwright E2E** — auth-redirect smoke test in `frontend/e2e/`.
...
Unified inbox across Gmail/Outlook/IMAP, account switching, compose / reply / forward / search, archive / delete / star / mark read, AI summary, AI priority with explainable factors, 3-tone reply drafts, classification, pattern detection, folder suggestions, semantic search (pgvector + HF embeddings), keyboard shortcuts, onboarding flow, encrypted IMAP credentials, PWA manifest.
...
- IMAP archive/trash at the IMAP layer (DB-side flag in MVP — see `lib/providers/imap.ts`)
...
Required env vars: `NEXTAUTH_SECRET`, `GOOGLE_CLIENT_ID`/`GOOGLE_CLIENT_SECRET`, `AZURE_AD_CLIENT_ID`/`AZURE_AD_CLIENT_SECRET`/`AZURE_AD_TENANT_ID`, `GROQ_API_KEY`, `HF_API_KEY`, `DATABASE_URL`/`DIRECT_URL`, `ENCRYPTION_KEY`. Full list in `frontend/.env.example`.
...
3. **Single source of truth.** AI calls go through `lib/ai/gemini.ts` or one of the seven agents. Don't fork prompts elsewhere.
