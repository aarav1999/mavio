...
export interface DrafterAgent {
  ...
  description: string;
  version: string;
  run(subject: string, threadBody: string, userIntent?: string): Promise<ReplyDraft[]>;
}

export const DrafterAgent: DrafterAgent = {
  name: 'DrafterAgent',
  description: 'Generates 3 reply drafts with different tones: professional, friendly, concise',
  version: '1.0.0',

  async run(subject: string, threadBody: string, userIntent?: string): Promise<ReplyDraft[]> {
